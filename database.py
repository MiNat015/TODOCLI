    ...
    with conn:
        c.execute(
            ...
            {'task': todo.task, 'category': todo.category,
             ...
             'date_completed': todo.date_completed, 'status': todo.status,
...
# def undo_complete_todo(position: int):
#     with conn:
#         c.execute(
#             'UPDATE todos SET status = 2, date_completed = :date_completed WHERE positon = :postion',
#             {
#                 'position': position,
#                 'date_completed': datetime.datetime.now().isoformat()
#             }
